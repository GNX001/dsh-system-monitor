...
export const DEFAULT_CONFIG = {
  ...
  /** Cadence for the network-throughput probe (spawns `typeperf` on Windows). */
  networkMs: 3000,
  ...
  cpuTemperature: true,
  /** Set false to skip network throughput entirely. */
  network: true,
  /**
   * Adapter name fragments excluded from the network total. Defaults to
   * `DEFAULT_EXCLUDED_INTERFACES` (loopback, VPN/overlay and virtual adapters),
   * which would otherwise double-count the same packets. Pass your own list to
   * override; the snapshot's `network.interfaces[].counted` shows what was used.
   */
  networkExclude: undefined,
  /** Adapter name fragments forced back in even if an exclusion matched. */
  networkInclude: undefined,
